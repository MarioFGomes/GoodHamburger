...
public class DeleteOrderUseCase : IDeleteOrderUseCase {
    ...
    public async Task ExecuteAsync(Guid id, CancellationToken ct = default) {
        ...
        // Soft delete keeps the full order (items included) for audit while the
        // global query filter hides it everywhere.
        order.MarkDeleted();
        await _unitOfWork.SaveChangesAsync(ct);

        _logger.LogInformation("Order soft-deleted. Id={OrderId}", id);
    }
}
